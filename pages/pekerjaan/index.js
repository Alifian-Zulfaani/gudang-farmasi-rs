...
import { getListPekerjaan, deletePekerjaan } from "api/pekerjaan";
...
const Pekerjaan = () => {
  ...
  const initData = async () => {
    try {
      ...
      const response = await getListPekerjaan(params);
  ...
  const updateDataHandler = async (payload) => {
    try {
      ...
      const response = await getListPekerjaan(payload);
      const result = dataFormatHandler(response.data.data);
      setData(result);
      setDataMeta(response.data.meta);
    } catch (error) {
  ...
  const deletaDataHandler = async (payload) => {
    try {
      setIsUpdatingData(true);
      const response = await deletePekerjaan({ id: payload });
      setSnackbarState({
        state: true,
        type: "success",
