...
import { getListSuku, deleteSuku } from "api/suku";
...
const Suku = () => {
  ...
  const initData = async () => {
    try {
      ...
      const response = await getListSuku(params);
  ...
  const updateDataHandler = async (payload) => {
    try {
      ...
      const response = await getListSuku(payload);
      const result = dataFormatHandler(response.data.data);
      setData(result);
      setDataMeta(response.data.meta);
    } catch (error) {
  ...
  const deletaDataHandler = async (payload) => {
    try {
      setIsUpdatingData(true);
      const response = await deleteSuku({ id: payload });
      setSnackbarState({
        state: true,
        type: "success",
