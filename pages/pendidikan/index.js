...
import { getListPendidikan, deletePendidikan } from "api/pendidikan";
...
const Pendidikan = () => {
  ...
  const initData = async () => {
    try {
      ...
      const response = await getListPendidikan(params);
  ...
  const updateDataHandler = async (payload) => {
    try {
      ...
      const response = await getListPendidikan(payload);
      const result = dataFormatHandler(response.data.data);
      setData(result);
      setDataMeta(response.data.meta);
    } catch (error) {
  ...
  const deletaDataHandler = async (payload) => {
    try {
      setIsUpdatingData(true);
      const response = await deletePendidikan({ id: payload });
      setSnackbarState({
        state: true,
        type: "success",
