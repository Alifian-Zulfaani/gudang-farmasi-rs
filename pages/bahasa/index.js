...
import { getListBahasa, deleteBahasa } from "api/bahasa";
...
const Bahasa = () => {
  ...
  const initData = async () => {
    try {
      ...
      const response = await getListBahasa(params);
  ...
  const updateDataHandler = async (payload) => {
    try {
      ...
      const response = await getListBahasa(payload);
      const result = dataFormatHandler(response.data.data);
      setData(result);
      setDataMeta(response.data.meta);
    } catch (error) {
  ...
  const deletaDataHandler = async (payload) => {
    try {
      setIsUpdatingData(true);
      const response = await deleteBahasa({ id: payload });
      setSnackbarState({
        state: true,
        type: "success",
