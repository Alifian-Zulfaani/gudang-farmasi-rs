...
const Form = ({
  ...
  const schema = Yup.object({
    name: stringSchema("Nama poliklinik", true),
  });
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (!isEditType) {
          await createPoliklinik(data);
          resetForm();
        } else {
          await updatePoliklinik({ ...data, id: detailPrePopulatedData.id });
          const response = await getDetailPoliklinik({
        ...
        <form onSubmit={validation.handleSubmit}>
          ...
          <div className="mt-16 flex justify-end items-center">
            <Button
              ...
              startIcon={<BackIcon />}
              sx={{ marginRight: 2 }}
              onClick={() => router.push("/poliklinik")}
            >
              Kembali
            </Button>
            {isEditType ? (
              <LoadingButton
                ...
                disabled={
                  ...
                  !isActionPermitted("poliklinik:update")
                }
                startIcon={<SaveIcon />}
              ...
              <LoadingButton
                type="submit"
                variant="contained"
                disabled={!isActionPermitted("poliklinik:store")}
                startIcon={<PlusIcon />}
                loadingPosition="start"
                loading={validation.isSubmitting}
              >
