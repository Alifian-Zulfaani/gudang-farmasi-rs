...
import { createBahasa, updateBahasa, getDetailBahasa } from "api/bahasa";
...
const Form = ({
  ...
  const schema = Yup.object({
    name: stringSchema("Nama bahasa", true),
  });
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (!isEditType) {
          await createBahasa(data);
          resetForm();
        } else {
          await updateBahasa({ ...data, id: detailPrePopulatedData.id });
          const response = await getDetailBahasa({
            ...
            {isEditType ? (
              <LoadingButton
                ...
                disabled={
                  ...
                  !isActionPermitted("bahasa:update")
                }
                startIcon={<SaveIcon />}
              ...
              <LoadingButton
                type="submit"
                variant="contained"
                disabled={!isActionPermitted("bahasa:store")}
                startIcon={<PlusIcon />}
                loadingPosition="start"
                loading={validation.isSubmitting}
              >
