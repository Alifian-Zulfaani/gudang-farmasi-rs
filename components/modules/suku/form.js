...
import { createSuku, updateSuku, getDetailSuku } from "api/suku";
...
const Form = ({
  ...
  const schema = Yup.object({
    name: stringSchema("Nama suku", true),
  });
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (!isEditType) {
          await createSuku(data);
          resetForm();
        } else {
          await updateSuku({ ...data, id: detailPrePopulatedData.id });
          const response = await getDetailSuku({
          ...
          <div className="mt-16 flex justify-end items-center">
            ...
            {isEditType ? (
              ...
              <LoadingButton
                type="submit"
                variant="contained"
                disabled={!isActionPermitted("suku:store")}
                startIcon={<PlusIcon />}
                loadingPosition="start"
                loading={validation.isSubmitting}
              >
