...
import { createAgama, updateAgama, getDetailAgama } from "api/agama";
...
const Form = ({
  ...
  const schema = Yup.object({
    name: stringSchema("Nama agama", true),
  });
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (!isEditType) {
          await createAgama(data);
          resetForm();
        } else {
          await updateAgama({ ...data, id: detailPrePopulatedData.id });
          const response = await getDetailAgama({
            ...
            {isEditType ? (
              <LoadingButton
                ...
                disabled={
                  ...
                  !isActionPermitted("agama:update")
                }
                startIcon={<SaveIcon />}
              ...
              <LoadingButton
                type="submit"
                variant="contained"
                disabled={!isActionPermitted("agama:store")}
                startIcon={<PlusIcon />}
                loadingPosition="start"
                loading={validation.isSubmitting}
              >
