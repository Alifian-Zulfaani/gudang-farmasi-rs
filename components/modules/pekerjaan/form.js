...
const Form = ({
  ...
  const schema = Yup.object({
    name: stringSchema("Nama pekerjaan", true),
  });
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (!isEditType) {
          await createPekerjaan(data);
          resetForm();
        } else {
          await updatePekerjaan({ ...data, id: detailPrePopulatedData.id });
          const response = await getDetailPekerjaan({
            ...
            {isEditType ? (
              <LoadingButton
                ...
                disabled={
                  ...
                  !isActionPermitted("pekerjaan:update")
                }
                startIcon={<SaveIcon />}
              ...
              <LoadingButton
                type="submit"
                variant="contained"
                disabled={!isActionPermitted("pekerjaan:store")}
                startIcon={<PlusIcon />}
                loadingPosition="start"
                loading={validation.isSubmitting}
              >
